...
just the right src image sizes and scale/rotation and can lead
...
Assumes dst surface was allocated with the correct sizes.
...
int _zoomSurfaceRGBA(SDL_Surface * src, SDL_Surface * dst, int flipx, int flipy, int smooth)
{
	...
	*/
	if (smooth) {
		/*
		* For interpolation: assume source size is one pixel
...
SDL_Surface *rotozoomSurfaceXY(SDL_Surface * src, double angle, double zoomx, double zoomy, int smooth)
{
	...
	*/
	iintbit = (src->format->BitsPerPixel == 32);
	if ((iintbit) || (src->format->BitsPerPixel == 8)) {
		/*
